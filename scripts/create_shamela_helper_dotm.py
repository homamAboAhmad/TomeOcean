"""
create_shamela_helper_dotm.py - إنشاء ملف shamela_helper.dotm
يقرأ كود VBA من dotm_code.txt وينشئ القالب
"""
...
def create_dotm():
    ...
    dotm_path = os.path.join(script_dir, "shamela_helper.dotm")
    ...
    try:
        ...
        try:
            ...
            print(f"  Go to: File > Options > Trust Center > Trust Center Settings > Macro Settings")
            print(f"  Check: 'Trust access to the VBA project object model'")
            print(f"  Error: {e}")
            try:
                doc.Close(False)
            except:
        ...
        # Save as .dotm (wdFormatXMLTemplateMacroEnabled = 13, but 15 also works for dotm)
        doc.SaveAs2(dotm_path, 13)
        try:
            doc.Close(0)
        except:
            pass
