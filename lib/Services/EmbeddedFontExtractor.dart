...
class EmbeddedFontExtractor {
  ...
  static Future<Map<String, String>> extractEmbeddedFonts(
    ...
    final sharedFontsDir = Directory(sharedFontsDirPath);
    ...
    for (var fontEl in fontTableDoc.findAllElements('w:font')) {
      ...
      for (String embedType in [
        ...
        String safeEmbedType = embedType.replaceAll(':', '_');
        String localPath = p.join(
          sharedFontsDir.path,
          '${safeFontName}_${fontHash}_$safeEmbedType.ttf',
        );
  ...
  static List<int> _deobfuscateFont(List<int> data, String? fontKey) {
    ...
    // ترتيب البايتات حسب مواصفات مصادر الخطوط المفتوحة ODTTF (ECMA-376 Part 4 Section 2.8.2.3)
    // المفتاح هو البايتات الـ 16 معكوسة الترتيب بالكامل
    List<int> keyBytes = guidBytes.reversed.toList();
